...
try:
    ...
    yf.pdr_override()  # Daha hızlı veri çekme
    YF_AVAILABLE = True
except ImportError:
    YF_AVAILABLE = False
...
# ================================
def get_xu100_data():
    """XU100 verisini çek - sadece TARA butonuna basıldığında çağrılır"""
    if not YF_AVAILABLE:
        return None, None, None
    try:
        xu100 = yf.Ticker("XU100.IS")
        data = xu100.history(period="6mo", interval="1wk")  # 6 ay yeterli
        if len(data) < 20:
            return None, None, None
        
        xu100_close = data['Close'].iloc[-1]
        xu100_ema50 = calculate_ema(data, 50).iloc[-1] if len(data) >= 50 else data['Close'].mean()
        
        return data, xu100_close, xu100_ema50
    except Exception as e:
        st.error(f"XU100 veri hatası: {e}")
        return None, None, None
...
def ramkar_scan_single(symbol: str, xu100_close, xu100_ema50) -> Tuple[Optional[Dict], bool]:
    try:
        ...
        data = stock.history(period="6mo", interval="1wk")  # 6 ay yeterli
